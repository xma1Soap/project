...
type textQuotaSummary struct {
	PromptTokens             int
	TotalInputTokens         int // 总输入tokens（含缓存命中/写入）
...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	...
	// 总输入tokens（含缓存命中与缓存写入）：anthropic 语义下 PromptTokens 不含缓存，需加回
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	// 使用本地计算的输入tokens覆盖上游返回值
	if relayInfo.ChannelSetting.UseLocalPromptTokens && usage != nil && relayInfo.GetEstimatePromptTokens() > 0 {
		localPrompt := relayInfo.GetEstimatePromptTokens()
		usage.PromptTokens = localPrompt
		usage.TotalTokens = localPrompt + usage.CompletionTokens
	}
	...
	emptyBilling := operation_setting.GetEmptyResponseBillingSetting()
	if summary.TotalTokens == 0 && !emptyBilling.Enabled {
		extraContent = append(extraContent, "上游没有返回计费信息，无法扣费（可能是上游超时）")
		logger.LogError(ctx, fmt.Sprintf("total tokens is 0, cannot consume quota, userId %d, channelId %d, tokenId %d, model %s， pre-consumed quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, summary.ModelName, relayInfo.FinalPreConsumedQuota))
	} else if summary.TotalTokens == 0 && emptyBilling.Enabled {
		extraContent = append(extraContent, "空回复扣费：completion_tokens=0，按 prompt_tokens 计费")
		logger.LogWarn(ctx, fmt.Sprintf("empty response billing: userId %d, channelId %d, tokenId %d, model %s, prompt_tokens %d, quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, summary.ModelName, summary.PromptTokens, summary.Quota))
	...
	// 测试惩罚：总输入tokens（含缓存）低于阈值时额外扣费
	testPenaltySetting := operation_setting.GetTestPenaltySetting()
	if testPenaltySetting.Enabled && !relayInfo.ChannelSetting.DisableTestPenalty && testPenaltySetting.MinPromptTokens > 0 && summary.TotalInputTokens < testPenaltySetting.MinPromptTokens && summary.TotalInputTokens > 0 {
		penaltyQuota := testPenaltySetting.PenaltyQuota
		if penaltyQuota > 0 {
			err := model.DecreaseUserQuota(relayInfo.UserId, penaltyQuota, false)
			if err != nil {
				logger.LogError(ctx, fmt.Sprintf("failed to apply test penalty: userId=%d, penalty=%d, err=%s", relayInfo.UserId, penaltyQuota, err.Error()))
			} else {
				extraContent = append(extraContent, fmt.Sprintf("测试惩罚扣费 %d (总输入tokens %d < %d)", penaltyQuota, summary.TotalInputTokens, testPenaltySetting.MinPromptTokens))
			}
		}
	}
