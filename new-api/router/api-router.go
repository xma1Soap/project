...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		channelRoute.Use(middleware.AdminAuth())
		{
			...
			channelRoute.GET("/quota-snapshot", middleware.RootAuth(), controller.GetChannelQuotaSnapshot)
			channelRoute.GET("/routes/:id", middleware.RootAuth(), controller.GetChannelRoutes)
			...
			channelRoute.PUT("/route", middleware.RootAuth(), controller.UpdateChannelRoute)
		...
		// 令牌命名审计（v8.6 定制功能）：只读扫描，自动封禁由宿主机 cron 执行
		apiRouter.GET("/token_name_audit", middleware.AdminAuth(), controller.GetTokenNameAudit)
