...
function ListSkeleton() {
  return (
    <div className='bg-card divide-y overflow-hidden rounded-lg border supports-[backdrop-filter]:backdrop-blur-xl'>
      {[1, 2, 3, 4, 5].map((i) => (
      ...
      {rows.map((row) => {
        ...
        return (
          <div
            key={key}
            className={cn(
              '[background-color:var(--data-table-card-bg,var(--table-row))] px-3 py-2.5 supports-[backdrop-filter]:backdrop-blur-xl',
              getRowClassName?.(row)
            )}
          >
