...
const buildFormDefaults = (defaults: FlatOAuthDefaults): OAuthFormValues => ({
  ...
  discord: {
    ...
    guild_id: defaults['discord.guild_id'] ?? '',
    role_ids: defaults['discord.role_ids'] ?? '',
  },
  oidc: {
...
const normalizeFormValues = (values: OAuthFormValues): FlatOAuthDefaults => ({
  ...
  'discord.guild_id': values.discord.guild_id,
  'discord.role_ids': values.discord.role_ids,
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value='discord' className={oauthTabContentClassName}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Discord Guild IDs')}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t('Optional comma-separated guild IDs')}
                          autoComplete='off'
                          value={field.value ?? ''}
                          onChange={(event) =>
                            field.onChange(event.target.value)
                      ...
                      <FormDescription>
                        {t(
                          'Users must belong to at least one configured Discord guild.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  control={form.control}
                  name='discord.role_ids'
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Discord Role IDs')}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t('Optional comma-separated role IDs')}
                          autoComplete='off'
                          value={field.value ?? ''}
                          onChange={(event) =>
                            field.onChange(event.target.value)
                      ...
                      <FormDescription>
                        {t(
                          'When role IDs are configured, users must have at least one matching role.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
