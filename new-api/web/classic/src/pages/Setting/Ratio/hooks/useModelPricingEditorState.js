...
export function useModelPricingEditorState({
  ...
  // v8.5: 页面加载(或上次保存刷新)时的 options 快照, 保存时用于计算条目级增量
  const loadedMapsRef = useRef(null);
  ...
  useEffect(() => {
    ...
    loadedMapsRef.current = sourceMaps;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // v8.5: 保存前拉取最新 options, 以「条目级增量」合并后再提交,
      // 防止用旧快照整表覆盖其他入口/其他标签页刚改的价格
      // (default UI 的模型抽屉已于 v8.4 修复, 此处补齐 classic 可视化编辑器入口)。
      try {
        const freshRes = await API.get('/api/option/');
        const freshEntries = freshRes?.data?.data;
        if (Array.isArray(freshEntries) && freshEntries.length > 0) {
          const freshValueByKey = {};
          freshEntries.forEach((item) => {
            freshValueByKey[item.key] = item.value;
          });
          const loadedMaps = loadedMapsRef.current || {};
          const mergeOutputMap = (outputKey, optionKey) => {
            const loaded = loadedMaps[outputKey] || {};
            const submitted = output[outputKey] || {};
            const fresh = parseOptionJSON(freshValueByKey[optionKey]);
            const merged = { ...fresh };
            ...
            output[outputKey] = merged;
          };
          mergeOutputMap('ModelPrice', 'ModelPrice');
          mergeOutputMap('ModelRatio', 'ModelRatio');
          mergeOutputMap('CompletionRatio', 'CompletionRatio');
          mergeOutputMap('CacheRatio', 'CacheRatio');
          mergeOutputMap('CreateCacheRatio', 'CreateCacheRatio');
          mergeOutputMap('ImageRatio', 'ImageRatio');
          mergeOutputMap('AudioRatio', 'AudioRatio');
          mergeOutputMap('AudioCompletionRatio', 'AudioCompletionRatio');

          const mergeTieredMap = (optionKey, loadedKey) => {
            const loaded = loadedMaps[loadedKey] || {};
            const submitted = tieredOutput[optionKey] || {};
            const fresh = parseOptionJSON(freshValueByKey[optionKey]);
            const merged = { ...fresh };
            const entryNames = new Set([
              ...Object.keys(loaded),
            ...
            tieredOutput[optionKey] = merged;
          };
          mergeTieredMap('billing_setting.billing_mode', 'ModelBillingMode');
          mergeTieredMap('billing_setting.billing_expr', 'ModelBillingExpr');
        }
      } catch (e) {
