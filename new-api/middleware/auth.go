...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// UA 黑名单检测（管理员和根用户豁免）
		if userCache.Role < common.RoleAdminUser {
			uaBlacklist := operation_setting.GetUABlacklistSetting()
			if uaBlacklist.Enabled && len(uaBlacklist.Keywords) > 0 {
				ua := c.Request.UserAgent()
				for _, keyword := range uaBlacklist.Keywords {
					if keyword != "" && strings.Contains(strings.ToLower(ua), strings.ToLower(keyword)) {
						// 直接禁用用户账号
						_ = model.DisableUser(token.UserId)
						// 仅管理员可见日志（user_id=0 隐藏于被封用户的「我的日志」，避免泄露黑名单关键词）
						logContent := fmt.Sprintf("UA黑名单自动封禁: 用户#%d(%s), ua=%s", token.UserId, userCache.Username, ua)
						common.SysLog(fmt.Sprintf("UA blacklist match: user %d, ua=%s", token.UserId, ua))
						model.RecordAdminOnlyLog(userCache.Username, ua, logContent)
						abortWithOpenAiMessage(c, http.StatusForbidden, "因使用不被允许的客户端，您的账户已被封禁，请联系管理员")
						return
					}
				}
			}
