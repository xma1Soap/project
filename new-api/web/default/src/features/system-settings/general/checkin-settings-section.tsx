...
const schema = z.object({
  ...
  quotaThreshold: z.coerce.number().int().min(0),
})
...
export function CheckinSettingsSection({
  ...
  const form = useForm<Values>({
    ...
    defaultValues: {
      ...
      quotaThreshold: defaultValues.quotaThreshold,
    },
  })
  ...
  async function onSubmit(values: Values) {
    ...
    if (values.quotaThreshold !== defaultValues.quotaThreshold) {
      updates.push({
        key: 'checkin_setting.quota_threshold',
        value: String(values.quotaThreshold),
      })
    }
        ...
        <SettingsForm onSubmit={form.handleSubmit(onSubmit)} autoComplete='off'>
          ...
          {enabled && (
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('Check-in quota threshold')}</FormLabel>
                  <FormControl>
                    <Input
                      type='number'
                      min={0}
                  ...
                  <FormDescription>
                    {t(
                      'Only users whose current quota is below this value can check in (0 = no limit). Guides low-quota users to check in.'
                    )}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
