...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int) ([]*User, int64, error) {
	...
	// 构建搜索条件（大小写不敏感的子串模糊匹配，LOWER 三库通用）
	// 除 username/email/display_name 外，同时支持按 Discord ID 搜索（管理员定位绑定 Discord 的用户）
	likeCondition := "LOWER(username) LIKE LOWER(?) OR LOWER(email) LIKE LOWER(?) OR LOWER(display_name) LIKE LOWER(?) OR LOWER(discord_id) LIKE LOWER(?)"
...
// DisableUser disables a user account by ID
func DisableUser(userId int) error {
	...
	var user User
	if dbErr := DB.First(&user, userId).Error; dbErr == nil {
