...
// normalizeImageCompletionTokens 修正图片输出的计费 token。
//
// 背景（生产事故 2026-06-23）：Gemini 图片模型（gemini-3-pro-image 等）经
// /v1/chat/completions 出图时，completion_tokens 会被灌成 100-180 万的巨值
// （根因是图片的 base64 数据被当文本估算 token，或上游把图片等效 token 混进
// candidatesTokenCount），且 candidatesTokensDetails 往往不给 IMAGE modality
// 拆分（实测日志 image_output 全空 → ImageTokens=0）。计费表达式
// tier("base", p*2 + c*12 + img_o*120) 里 c 取 CompletionTokens、img_o 取
// ImageTokens：c 含整张图、img_o=0、tiered_settle 的 `c -= imgO` 减不掉
// → 图片按文本 c*12 计费 → 单图被扣 $5-$20（应为几分钱）。
//
// 修正：图片输出的计费 token 必须由"图片张数 × 固定 258 token"决定
// （Gemini 官方 cookbook 值），与上游/本地估算的巨值无关。当响应含图片输出时，
// 把 ImageTokens 设为 imageCount*258，并把 CompletionTokens 中**非文本**的部分
// （即被污染的图片 token）剔除，使 c 回归真实文本量、img_o 拿到正确图片 token。
// 文本 completion 的判定：若上游给了 TEXT modality 拆分则用它，否则视为 0
// （纯出图请求通常无伴随文本，少量文本损失可忽略，远优于按百万 token 爆扣）。
func normalizeImageCompletionTokens(usage *dto.Usage, imageCount int) {
	if usage == nil || imageCount <= 0 {
		return
	}
	// 上游已正确拆分 IMAGE modality 且 completion 未被污染，无需修正。
	if usage.CompletionTokenDetails.ImageTokens > 0 {
		return
	}
	const perImageTokens = 258 // Gemini 官方：每张生成图固定 258 token
	usage.CompletionTokenDetails.ImageTokens = imageCount * perImageTokens
	// 真实文本 completion：优先用上游 TEXT modality 拆分；没有则视为 0，
	// 丢弃被 base64/图片污染的 CompletionTokens 巨值（这是爆扣的根源）。
	textCompletion := usage.CompletionTokenDetails.TextTokens
	usage.CompletionTokens = textCompletion + usage.CompletionTokenDetails.ImageTokens
}
...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, *types.NewAPIError) {
	...
	if imageCount != 0 {
		// 图片输出按固定 258/张计费，剔除被污染的巨值 completion（生产事故 2026-06-23）
		normalizeImageCompletionTokens(usage, imageCount)
	}
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// 统计图片输出数，修正图片计费 token（生产事故 2026-06-23：出图 completion 爆扣）
	imageCount := 0
	for _, candidate := range geminiResponse.Candidates {
		for _, part := range candidate.Content.Parts {
