    ...
    try {
      await refForm.current
        ...
        .then(async () => {
          ...
          // 合并规则: 只把「相对页面加载快照(inputsRow)」变化的条目套到最新值上。
          ...
          try {
            ...
            if (Array.isArray(freshEntries) && freshEntries.length > 0) {
              ...
              for (const item of updateArray) {
                if (typeof inputs[item.key] !== 'string') continue;
                const freshRaw = freshValueByKey[item.key];
                if (freshRaw === undefined) continue;
                const loaded = parseMap(inputsRow[item.key]);
                const submitted = parseMap(inputs[item.key]);
                ...
                mergedInputs[item.key] = JSON.stringify(merged, null, 2);
              }
            }
          } catch (e) {
          ...
          const requestQueue = updateArray.map((item) => {
            const value =
              typeof mergedInputs[item.key] === 'boolean'
                ? String(mergedInputs[item.key])
                : mergedInputs[item.key];
