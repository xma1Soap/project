...
func InitChannelCache() {
	...
	for _, ability := range abilities {
		...
		if !exists || channel.Status != common.ChannelStatusEnabled || !ability.Enabled {
			continue
		}
...
func GetRandomSatisfiedChannel(group string, model string, retry int, requestPath string) (*Channel, error) {
	...
	// skip channels in cooldown (recently errored) when the feature is on
	if setting.ChannelCooldownEnabled && len(channels) > 0 {
		channels = filterChannelsNotInCooldown(channels, model)
	}
...
// ── channel cooldown ─────────────────────────────────────────────
// Skip routing to a channel for N seconds after it errors (2026-08-06).
type channelModelCooldownKey struct {
	channelID int
	model     string
}

var channelModelCooldown = struct {
	sync.RWMutex
	m map[channelModelCooldownKey]int64
}{m: make(map[channelModelCooldownKey]int64)}

func cooldownKey(channelID int, modelName string) channelModelCooldownKey {
	modelName = ratio_setting.FormatMatchingModelName(strings.TrimSpace(modelName))
	return channelModelCooldownKey{channelID: channelID, model: modelName}
}

// SetChannelModelCooldown temporarily removes one route from request selection.
func SetChannelModelCooldown(channelID int, modelName string, seconds int64) {
	key := cooldownKey(channelID, modelName)
	if key.channelID <= 0 || key.model == "" || seconds <= 0 {
		return
	}
	channelModelCooldown.Lock()
	defer channelModelCooldown.Unlock()
	channelModelCooldown.m[key] = time.Now().Unix() + seconds
}

func isChannelModelInCooldown(channelID int, modelName string) bool {
	key := cooldownKey(channelID, modelName)
	channelModelCooldown.RLock()
	end, ok := channelModelCooldown.m[key]
	channelModelCooldown.RUnlock()
	if !ok {
		return false
	}
	...
	channelModelCooldown.Lock()
	if channelModelCooldown.m[key] <= time.Now().Unix() {
		delete(channelModelCooldown.m, key)
	}
	channelModelCooldown.Unlock()
	return false
}

func filterChannelsNotInCooldown(channels []int, modelName string) []int {
	out := make([]int, 0, len(channels))
	for _, id := range channels {
		if !isChannelModelInCooldown(id, modelName) {
			out = append(out, id)
		}
	}
	return out
