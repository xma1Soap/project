...
func migrateDB() error {
	...
	// Migrate logs.user_agent column from varchar(512) to text for existing tables
	// (shared LOG_DB case: the logs table lives in the main DB; 非 ClickHouse)
	if !common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		if err := migrateLogUserAgentToText(DB); err != nil {
			return err
		}
		// 删除运营确认不用的废日志索引(request_id/upstream_request_id) + PG 专属 pg_trgm UA 索引。
		// best-effort，不阻断启动；PG 专属部分按方言门控，三库兼容。
		migrateLogIndexCleanup(DB)
	}
	return nil
}
...
func migrateLOGDB() error {
	...
	// Migrate logs.user_agent column from varchar(512) to text for existing
	// separate LOG_SQL_DSN databases (非 ClickHouse)
	if err := migrateLogUserAgentToText(LOG_DB); err != nil {
		return err
	}
	migrateLogIndexCleanup(LOG_DB)
	return nil
}
...
func migrateLogIndexCleanup(db *gorm.DB) {
	if db == nil {
		return
	}
	m := db.Migrator()
	...
	dropIdx := []string{"idx_logs_request_id", "idx_logs_upstream_request_id"}
	switch string(db.Dialector.Name()) {
	case string(common.DatabaseTypePostgreSQL), string(common.DatabaseTypeSQLite):
		for _, idx := range dropIdx {
			if err := db.Exec("DROP INDEX IF EXISTS " + idx).Error; err != nil {
				common.SysLog(fmt.Sprintf("migrateLogIndexCleanup: drop %s: %v", idx, err))
			}
		}
	case string(common.DatabaseTypeMySQL):
		for _, idx := range dropIdx {
			if m.HasIndex(&Log{}, idx) {
				if err := m.DropIndex(&Log{}, idx); err != nil {
					common.SysLog(fmt.Sprintf("migrateLogIndexCleanup: drop %s: %v", idx, err))
				}
			}
		}
	}
	if string(db.Dialector.Name()) == string(common.DatabaseTypePostgreSQL) {
		if err := db.Exec("CREATE EXTENSION IF NOT EXISTS pg_trgm").Error; err != nil {
			common.SysLog("migrateLogIndexCleanup: CREATE EXTENSION pg_trgm failed (日志文本搜索保持扫描): " + err.Error())
		} else {
			// 为大小写不敏感的模糊日志搜索（LOWER(col) LIKE）建 pg_trgm GIN 索引，
			// 让管理员对 user_agent/content/model_name/token_name 的子串搜索从全表扫描变索引查找。
			// 全部 best-effort，失败仅告警不阻断启动；其它库这些搜索保持原样。
			trgmIdx := map[string]string{
				"idx_logs_ua_trgm":         "user_agent",
				"idx_logs_content_trgm":    "content",
				"idx_logs_model_name_trgm": "model_name",
				"idx_logs_token_name_trgm": "token_name",
			}
			for idxName, col := range trgmIdx {
				stmt := "CREATE INDEX IF NOT EXISTS " + idxName + " ON logs USING gin (lower(" + col + ") gin_trgm_ops)"
				if err := db.Exec(stmt).Error; err != nil {
					common.SysLog("migrateLogIndexCleanup: create " + idxName + " failed: " + err.Error())
				}
			}
		}
	}
...
func migrateLogUserAgentToText(db *gorm.DB) error {
	if db == nil {
		return nil
	}
	...
	if !db.Migrator().HasColumn(&Log{}, columnName) {
		return nil
	}
