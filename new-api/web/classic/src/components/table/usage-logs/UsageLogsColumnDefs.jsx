    ...
    },
    {
      ...
      render: (text, record, index) => {
        const showIp =
          ...
          text;
        if (!showIp) return <></>;
        // Show real IP only to root users. Other users (incl. sub-admins) see masked value.
        // DB still records the real IP for backend auditing.
        const hideIp = !isRootUser;
        const displayText = hideIp ? '••••••••' : text;
        const tooltipContent = hideIp ? t('此用户的 IP 已隐藏') : text;
        const tagColor = hideIp ? 'grey' : 'orange';
        return (
