  ...
  return (
    <div className={props.gridClassName ?? DEFAULT_GRID_CLASSNAME}>
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <div
          key={`${prefix}-${i}`}
          className='space-y-3 rounded-lg border bg-(--table-row) p-3 supports-[backdrop-filter]:backdrop-blur-xl'
        >
...
export function DataTableCardGrid<TData>(props: DataTableCardGridProps<TData>) {
  ...
  if (!rows || rows.length === 0) {
    return (
      <div className='bg-card rounded-lg border p-6 supports-[backdrop-filter]:backdrop-blur-xl'>
      ...
      {rows.map((row) => {
        ...
        return (
          <div
            ...
            className={cn(
              'rounded-lg border bg-(--data-table-card-bg,var(--table-row)) px-3 py-2.5 transition-[background-color,border-color] duration-150 data-[state=selected]:[--data-table-card-bg:color-mix(in_oklch,var(--primary)_7%,var(--table-row))] data-[state=selected]:border-primary/40 supports-[backdrop-filter]:backdrop-blur-xl',
